...
| `agents/` | One subfolder per agent, four-file pattern (`agent.py`, `agent_card.json`, `INSTRUCTIONS.md`, plus one of `config.yaml` / `config.example.yaml`). See "Per-agent `config.yaml` schema" below for the candidate-resolution convention. | 🟡 F3.7.1 (orchestrator scaffolded); F3.7.2 (real INSTRUCTIONS.md + 4-skill A2A contract published in `agent_card.json` — `status` still `in_development` until tool implementations land); F3.7.3-3.7.6 wire the tools (`list_available_specialists` → `delegate_to_specialist` → `check_task_status` → `request_human_approval`) and flip `status` to `available`; F3.8 (execution-agent vertical slice); F3.9 (stubs); F3.10 (promotions) |
